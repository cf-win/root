...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default="Beauty_5", help='dataset')
    parser.add_argument("--root_path", type=str, default="../checkpoint/Beauty_5/", help='root path')
    parser.add_argument('--alpha', type=str, default='1e-2', help='cf loss weight')
...
ckpt_candidates = [
    args_setting.root_path + args_setting.checkpoint,
    args_setting.root_path + f'alpha{args_setting.alpha}-beta{args_setting.beta}/' + args_setting.checkpoint,
]
...
if ckpt_path is None:
    # Try to find any checkpoint in root_path
    import glob
    ckpt_files = glob.glob(args_setting.root_path + "**/*.pth", recursive=True)
    if ckpt_files:
        ckpt_path = ckpt_files[0]
    else:
        raise FileNotFoundError(f"No checkpoint found in {args_setting.root_path}")

print(f"Using checkpoint: {ckpt_path}")

output_dir = args_setting.output_dir if args_setting.output_dir else f"../data/{dataset}/"
