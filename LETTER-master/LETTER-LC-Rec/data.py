...
class BaseDataset(Dataset):
    ...
    @staticmethod
    def _valid_seq(items, min_len=3):
        return items is not None and len(items) >= min_len
...
class SeqRecDataset(BaseDataset):
    ...
    def __init__(self, args, mode="train",
        ...
        self.target_k = getattr(args, "target_k", 1)
    ...
    def _remap_items(self):
        ...
        for uid, items in self.inters.items():
            new_items = ["".join(self.indices[str(i)]) for i in items if str(i) in self.indices]
            self.remapped_inters[uid] = new_items

    def _process_train_data(self):
        inter_data = []
        reserve = self.target_k + 1
        for uid in self.remapped_inters:
            full_items = self.remapped_inters[uid]
            if not self._valid_seq(full_items, min_len=reserve + 1):
                continue

            items = full_items[:-reserve]
    ...
    def _process_valid_data(self):
        ...
        for uid in self.remapped_inters:
            ...
            if not self._valid_seq(items, min_len=reserve + 1):
                continue

            one_data = dict()
            one_data["item"] = items[-reserve]
            history = items[:-reserve]
            if self.max_his_len > 0:
                history = history[-self.max_his_len:]
            if self.add_prefix:
                history = [str(k + 1) + ". " + item_idx for k, item_idx in enumerate(history)]
    ...
    def _process_test_data(self):
        ...
        if self.sample_num > 0 and len(inter_data) > self.sample_num:
            all_inter_idx = range(len(inter_data))
            sample_idx = np.random.choice(list(all_inter_idx), self.sample_num, replace=False)
    ...
    def _construct_valid_text(self):
        ...
        if self.sample_valid:
            ...
            for i in range(len(self.inter_data)):
                d = self.inter_data[i]
                prompt_ids = np.random.choice(list(all_prompt_ids), self.prompt_sample_num, replace=False)
...
class FusionSeqRecDataset(BaseDataset):
    ...
    def _process_train_data(self):
        ...
        for uid in self.inters:
            full_items = self.inters[uid]
            if not self._valid_seq(full_items, min_len=3):
                continue

            items = full_items[:-2]
            for i in range(1, len(items)):
                if str(items[i]) not in self.indices or str(items[i]) not in self.item_feat:
                    continue
                ...
                valid_history = [j for j in history if str(j) in self.indices and str(j) in self.item_feat]
                inters = ["".join(self.indices[str(j)]) for j in valid_history]
                inter_titles = ['"' + self.item_feat[str(j)]["title"].strip().strip(".!?,;:`") + '"' for j in valid_history]
    ...
    def _process_valid_data(self):
        ...
        for uid in self.inters:
            ...
            if not self._valid_seq(items, min_len=3):
                continue
            if str(items[-2]) not in self.indices or str(items[-2]) not in self.item_feat:
                continue
    ...
    def _process_test_data(self):
        ...
        for uid in self.inters:
            ...
            if not self._valid_seq(items, min_len=3):
                continue
            if str(items[-1]) not in self.indices or str(items[-1]) not in self.item_feat:
                continue
...
class ItemFeatDataset(BaseDataset):
    ...
    def _process_data(self):
        ...
        if self.sample_num > 0 and len(feat_data) > self.sample_num:
            ...
            sample_idx = np.random.choice(list(all_idx), self.sample_num, replace=False)
            feat_data = np.array(feat_data, dtype=object)[sample_idx].tolist()
...
class ItemSearchDataset(BaseDataset):
    ...
    def _process_data(self):
        ...
        for uid in user_explicit_preference.keys():
            ...
            if str(iid) not in self.indices:
                continue
            ...
            inters = ["".join(self.indices[str(i)]) for i in inters if str(i) in self.indices]
        ...
        if self.sample_num > 0 and len(search_data) > self.sample_num:
            all_idx = range(len(search_data))
            sample_idx = np.random.choice(list(all_idx), self.sample_num, replace=False)
            search_data = np.array(search_data, dtype=object)[sample_idx].tolist()
...
class PreferenceObtainDataset(BaseDataset):
    ...
    def _process_data(self):
        ...
        for uid in user_explicit_preference.keys():
            if uid not in self.remapped_inters:
                continue
            if not self._valid_seq(self.remapped_inters[uid], min_len=3):
                continue
        ...
        if self.sample_num > 0 and len(preference_data) > self.sample_num:
            all_idx = range(len(preference_data))
            sample_idx = np.random.choice(list(all_idx), self.sample_num, replace=False)
            preference_data = np.array(preference_data, dtype=object)[sample_idx].tolist()
...
class SeqRecTestDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        return dict(input_ids=input_text, labels=target)